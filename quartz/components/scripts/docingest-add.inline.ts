const DOCINGEST_ADD_API_URL = "https://docingest.iamrp.dev/api"
...
document.addEventListener("nav", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      try {
        const checkRes = await fetch(`${DOCINGEST_ADD_API_URL}/docs/check-domain/${encodedUrl}`)
        if (checkRes.ok) {
          const checkData = await checkRes.json()
          if (checkData.found) {
            log(
              `Existing documentation found. (Last updated: ${new Date(checkData.lastUpdated).toLocaleDateString()})`,
            )
          }
        }
      } catch (err) {
      ...
      const response = await fetch(`${DOCINGEST_ADD_API_URL}/crawl/start`, {
        method: "POST",
        headers: {
  ...
  async function pollCrawlStatus(id: string, domain: string, url: string, attempts = 0) {
    ...
    try {
      const res = await fetch(`${DOCINGEST_ADD_API_URL}/crawl/status/${id}`)
      if (!res.ok) {
        if (res.status === 429) {
      ...
      if (data.status === "completed") {
        ...
        const saveRes = await fetch(`${DOCINGEST_ADD_API_URL}/docs/save`, {
          method: "POST",
